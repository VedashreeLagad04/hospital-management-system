import { Component, OnInit } from '@angular/core';
import { Router } from '@angular/router';
import { SocialSharing } from '@ionic-native/social-sharing/ngx';
import { ModalController } from '@ionic/angular';
import html2pdf from 'html2pdf.js';
import { AppDataService } from 'src/app/services/app-data.service';
import { IspCalculatorPage } from './isp-calculator/isp-calculator.page';
import { environment } from 'src/environments/environment';
@Component({
  selector: 'app-isp-calculation',
  templateUrl: './isp-calculation.page.html',
  styleUrls: ['./isp-calculation.page.scss'],
})
export class IspCalculationPage implements OnInit {
  public isWeb = environment.isWeb;
  public showTypeDropdown: any = [];
  public closeBreakdown = false;
  public showAdditionalPlan = false;
  public withdrawalLimit;
  public billSize: any;
  public proRationFactor;
  public afterProRationFactor;
  public basicDeductible;
  public riderBasicDeductible;
  public basicCoInsurance;
  public basicPayout;
  public nonPayableDeductible;
  public nonPayableCoInsurance;
  public coPayment;
  public riderPayout;
  public medicalReimbursement = '0';
  public mobilityAids = '0';
  public dailyAllowance = '0';
  public weeklyIndemnity = '0';
  public claimableAmount;
  public totalPayoutByInsurer;
  public nonPaymentByInsurer;
  public cashExposure;
  public planType = 'Personal Accident Plan';
  public regExp = new RegExp('(\\.[\\d]{2}).', 'g');
  public calculatorData: any;
  public file: any;
  public fileName: any;
  public internetCheckFlag = false;
  public factor: any;
  public payout: any;
  public additionalPlans: any = [];
  // tslint:disable-next-line: max-line-length
  constructor (private dataService: AppDataService, private modalController: ModalController, private router: Router, private socialSharing: SocialSharing) { }
  public ionViewDidEnter() {
    this.dataService.present().then((loader) => {
      loader.present();
      const loggedInUser = this.dataService.getUserData();
      // ? publish the header title you want to display in header
      const obj = {
        title: 'ISP Calculation',
        backPage: '/user-home/' + loggedInUser.id,
      };
      this.dataService.setHeaderTitle(obj);
      this.dataService.dismiss();
    });
  }
  public ngOnInit() {
    this.dataService.mySubscription = this.dataService.createOnline$().subscribe(async (isOnline) => {
      if (isOnline === false) {
        this.internetCheckFlag = true;
        this.dataService.toastPresent('Internet disconnected');
      } else if (isOnline === true && this.internetCheckFlag) {
        this.router.routeReuseStrategy.shouldReuseRoute = () => {
          return false;
        };
        this.ionViewDidEnter();
        this.dataService.toastPresent('Internet Connected');
        this.internetCheckFlag = false;
      }
    });
  }
  // tslint:disable-next-line: use-lifecycle-interface
  public ngOnDestroy() {
    if (this.dataService.mySubscription) {
      this.dataService.mySubscription.unsubscribe();
    }
  }
  public openTypeDropdown(i) {
    this.additionalPlans[i].showTypeDropdown = !this.additionalPlans[i].showTypeDropdown;
  }
  public async openCalculator() {
    const modal = await this.modalController.create({
      component: IspCalculatorPage,
      // componentProps: ,
      cssClass: 'calculator-modal',
      keyboardClose: true,
      // showBackdrop: true,
      backdropDismiss: false,
    }).then((modalEl1) => {
      modalEl1.present();
      modalEl1.onDidDismiss().then((resp) => {
        if (resp.data.type) {
          this.withdrawalLimit = resp.data.withdrawalLimit;
          this.calculatorData = resp.data;
          if (this.nonPaymentByInsurer) {
            // this.cashExposure = this.nonPaymentByInsurer - this.withdrawalLimit;
            const total = this.nonPaymentByInsurer - this.withdrawalLimit;
            this.cashExposure = total.toFixed(2);
          }
        }
      });
    });
  }
  public closeViewBreakdown(mode) {
    if (mode === 'close') {
      this.closeBreakdown = !this.closeBreakdown;
      this.showAdditionalPlan = false;
    } else if (mode === 'view') {
      this.closeBreakdown = !this.closeBreakdown;
      this.showAdditionalPlan = true;
    }
  }
  public toShowAdditionalPlan() {
    this.showAdditionalPlan = !this.showAdditionalPlan;
  }
  public addAdditionalPlan() {
    const newPlan = {
      planType: 'Personal Accident Plan',
      medicalReimbursement: '',
      mobilityAids: '',
      dailyAllowance: '',
      weeklyIndemnity: '',
      claimableAmount: 0,
      dailyAccidentalInput: '',
      dailyAccidentalIncomePayout: 0,
      dailyIncomePayout: 0,
      weeklyPayout: 0,
      dailyIncomeFactor: '',
      dailyAccidentalFactor: '',
      weeklyFactor: '',
      getWellBenefit: '',
      showTypeDropdown: false,
    };
    this.additionalPlans.push(newPlan);
  }
  public calculatePayoutAmount(i, type) {
    if (type === 'daily' && this.additionalPlans[i].dailyIncomeFactor && this.additionalPlans[i].dailyIncomeFactor.length !== 0) {
      this.additionalPlans[i].dailyIncomePayout = (parseFloat(this.additionalPlans[i].dailyIncomeFactor) * parseFloat(this.additionalPlans[i].dailyAllowance)).toFixed(2);
    } else if (type === 'accidental' && this.additionalPlans[i].dailyAccidentalFactor && this.additionalPlans[i].dailyAccidentalFactor.length !== 0) {
      this.additionalPlans[i].dailyAccidentalIncomePayout = (parseFloat(this.additionalPlans[i].dailyAccidentalFactor) * parseFloat(this.additionalPlans[i].dailyAccidentalInput)).toFixed(2);
    } else if (type === 'weekly' && this.additionalPlans[i].weeklyFactor && this.additionalPlans[i].weeklyFactor.length !== 0) {
      this.additionalPlans[i].weeklyPayout = (parseFloat(this.additionalPlans[i].weeklyFactor) * parseFloat(this.additionalPlans[i].weeklyIndemnity)).toFixed(2);
    }
    this.calculateClaimableAmount(i);
  }
  public deleteAdditionalPlan(i) {
    this.additionalPlans.splice(i, 1);
  }
  public selectPlanType(type, i) {
    this.additionalPlans[i].planType = type;
    if (this.additionalPlans[i].planType !== 'Personal Accident Plan') {
      this.additionalPlans[i].mobilityAids = 0;
    }
    this.calculateClaimableAmount(i);
  }
  public calculateAfterProRationFactor() {
    if ((this.billSize !== undefined && this.billSize !== '') && (this.proRationFactor !== undefined && this.proRationFactor !== '')) {
      if (this.proRationFactor && this.proRationFactor.toString().match(/^0(?:\.0+)?$/)) {
        this.afterProRationFactor = 0;
      } else {
        // this.proRationFactor = parseFloat(this.proRationFactor).toFixed(2);
        this.afterProRationFactor = (this.billSize * (this.proRationFactor / 100)).toFixed(2);
        this.calculateBasicCoInsuranceBasicPayout();
      }
    }
  }
  public calculateBasicCoInsuranceBasicPayout() {
    // tslint:disable-next-line: max-line-length
    if ((this.afterProRationFactor !== undefined && this.afterProRationFactor !== '') && (this.basicDeductible !== undefined && this.basicDeductible !== '')) {
      this.basicCoInsurance = ((this.afterProRationFactor - this.basicDeductible) * 0.1).toFixed(2);
      if (this.basicCoInsurance !== undefined || this.basicCoInsurance !== '') {
        const temp = this.afterProRationFactor - this.basicDeductible - this.basicCoInsurance;
        this.basicPayout = temp.toFixed(2);
      }
    }
  }
  public calculateRiderPayout() {
    // tslint:disable-next-line: max-line-length
    if ((this.basicDeductible !== undefined && this.basicDeductible !== '') && (this.basicCoInsurance !== undefined && this.basicCoInsurance !== '') && (this.nonPayableDeductible !== undefined && this.nonPayableDeductible !== '') && (this.nonPayableCoInsurance !== undefined && this.nonPayableCoInsurance !== '') && (this.coPayment !== undefined && this.coPayment !== '')) {
      // tslint:disable-next-line: max-line-length
      const temp = (parseFloat(this.basicDeductible) + parseFloat(this.basicCoInsurance)) - parseFloat(this.nonPayableDeductible) - parseFloat(this.nonPayableCoInsurance) - parseFloat(this.coPayment);
      this.riderPayout = temp.toFixed(2);
    }
    if (this.basicPayout !== undefined || this.basicPayout !== '') {
      const temp = parseFloat(this.basicPayout) + parseFloat(this.riderPayout);
      this.totalPayoutByInsurer = temp.toFixed(2);
    }
    if (this.billSize !== undefined || this.billSize !== '') {
      const temp = parseFloat(this.billSize) - parseFloat(this.totalPayoutByInsurer);
      this.nonPaymentByInsurer = temp.toFixed(2);
    }
  }
  public calculateClaimableAmount(i) {
    this.additionalPlans[i].claimableAmount = 0;
    this.additionalPlans[i].claimableAmount = this.fixedDigits(this.additionalPlans[i].medicalReimbursement) +
      this.fixedDigits(this.additionalPlans[i].mobilityAids) +
      this.fixedDigits(this.additionalPlans[i].dailyIncomePayout) +
      this.fixedDigits(this.additionalPlans[i].dailyAccidentalIncomePayout) +
      this.fixedDigits(this.additionalPlans[i].weeklyPayout) +
      this.fixedDigits(this.additionalPlans[i].getWellBenefit)
    this.additionalPlans[i].claimableAmount = this.fixedDigits(this.additionalPlans[i].claimableAmount);
  }
  fixedDigits(num) {
    if (num || num.length !== 0) {
      return parseFloat(num);
    } else {
      return 0;
    }
  }
  public calculateCashExposure() {
    // tslint:disable-next-line: max-line-length
    if ((this.nonPaymentByInsurer !== undefined && this.nonPaymentByInsurer !== '') && (this.withdrawalLimit !== undefined && this.withdrawalLimit !== '')) {
      const temp = parseFloat(this.nonPaymentByInsurer) - parseFloat(this.withdrawalLimit);
      this.cashExposure = temp.toFixed(2);
    }
  }
  public trimInput() {
    if (this.billSize && this.billSize !== '') {
      const temp = parseFloat(this.billSize);
      this.billSize = temp.toFixed(2);
    }
    if (this.basicDeductible && this.basicDeductible !== '') {
      const temp = parseFloat(this.basicDeductible);
      this.basicDeductible = temp.toFixed(2);
    }
    if (this.nonPayableDeductible && this.nonPayableDeductible !== '') {
      const temp = parseFloat(this.nonPayableDeductible);
      this.nonPayableDeductible = temp.toFixed(2);
    }
    if (this.nonPayableCoInsurance && this.nonPayableCoInsurance !== '') {
      const temp = parseFloat(this.nonPayableCoInsurance);
      this.nonPayableCoInsurance = temp.toFixed(2);
    }
    if (this.coPayment && this.coPayment !== '') {
      const temp = parseFloat(this.coPayment);
      this.coPayment = temp.toFixed(2);
    }
    if (this.basicDeductible && this.basicDeductible !== '') {
      const temp = parseFloat(this.basicDeductible);
      this.basicDeductible = temp.toFixed(2);
    }
    if (this.basicCoInsurance && this.basicCoInsurance !== '') {
      const temp = parseFloat(this.basicCoInsurance);
      this.basicCoInsurance = temp.toFixed(2);
    }
    if (this.riderPayout && this.riderPayout !== '') {
      const temp = parseFloat(this.riderPayout);
      this.riderPayout = temp.toFixed(2);
    }
    // if (this.medicalReimbursement && this.medicalReimbursement !== '') {
    //   const temp = parseFloat(this.medicalReimbursement);
    //   this.medicalReimbursement = temp.toFixed(2);
    // }
    // if (this.mobilityAids && this.mobilityAids !== '') {
    //   const temp = parseFloat(this.mobilityAids);
    //   this.mobilityAids = temp.toFixed(2);
    // }
    // if (this.dailyAllowance && this.dailyAllowance !== '') {
    //   const temp = parseFloat(this.dailyAllowance);
    //   this.dailyAllowance = temp.toFixed(2);
    // }
    // if (this.weeklyIndemnity && this.weeklyIndemnity !== '') {
    //   const temp = parseFloat(this.weeklyIndemnity);
    //   this.weeklyIndemnity = temp.toFixed(2);
    // }
    // if (this.claimableAmount && this.claimableAmount !== '') {
    //   const temp = parseFloat(this.claimableAmount);
    //   this.claimableAmount = temp.toFixed(2);
    // }
    if (this.withdrawalLimit && this.withdrawalLimit !== '') {
      const temp = parseFloat(this.withdrawalLimit);
      this.withdrawalLimit = temp.toFixed(2);
    }
    if (this.totalPayoutByInsurer && this.totalPayoutByInsurer !== '') {
      const temp = parseFloat(this.totalPayoutByInsurer);
      this.totalPayoutByInsurer = temp.toFixed(2);
    }
    if (this.nonPaymentByInsurer && this.nonPaymentByInsurer !== '') {
      const temp = parseFloat(this.nonPaymentByInsurer);
      this.nonPaymentByInsurer = temp.toFixed(2);
    }
    if (this.cashExposure && this.cashExposure !== '') {
      const temp = parseFloat(this.cashExposure);
      this.cashExposure = temp.toFixed(2);
    }
  }
  trimNewPlan(i, type) {
    if (type === 'medicalReimbursement') {
      if (this.additionalPlans[i].medicalReimbursement === undefined && this.additionalPlans[i].medicalReimbursement.length === 0) {
        this.additionalPlans[i].medicalReimbursement = 0;
      } else {
        this.additionalPlans[i].medicalReimbursement = parseFloat(this.additionalPlans[i].medicalReimbursement).toFixed(2);
      }
    }
    if (type === 'mobilityAids') {
      if (this.additionalPlans[i].planType !== 'Personal Accident Plan') {
        this.additionalPlans[i].mobilityAids = 0;
      } else if (this.additionalPlans[i].planType === 'Personal Accident Plan' && !this.additionalPlans[i].mobilityAids && this.additionalPlans[i].mobilityAids.length === 0) {
        this.additionalPlans[i].mobilityAids = 0;
      } else {
        this.additionalPlans[i].mobilityAids = parseFloat(this.additionalPlans[i].mobilityAids).toFixed(2);
      }
    }
    if (type === 'dailyAllowance') {
      if (!this.additionalPlans[i].dailyAllowance || this.additionalPlans[i].dailyAllowance.length === 0) {
        this.additionalPlans[i].dailyAllowance = 0;
      } else {
        this.additionalPlans[i].dailyAllowance = parseFloat(this.additionalPlans[i].dailyAllowance).toFixed(2);
        this.calculatePayoutAmount(i, 'daily');
      }
    }
    if (type === 'dailyAccidentalInput') {
      if (!this.additionalPlans[i].dailyAccidentalInput || this.additionalPlans[i].dailyAccidentalInput.length === 0) {
        this.additionalPlans[i].dailyAccidentalInput = 0;
      } else {
        this.additionalPlans[i].dailyAccidentalInput = parseFloat(this.additionalPlans[i].dailyAccidentalInput).toFixed(2);
        this.calculatePayoutAmount(i, 'accidental');
      }
    }
    if (type === 'weeklyIndemnity') {
      if (!this.additionalPlans[i].weeklyIndemnity || this.additionalPlans[i].weeklyIndemnity.length === 0) {
        this.additionalPlans[i].weeklyIndemnity = 0;
      } else {
        this.additionalPlans[i].weeklyIndemnity = parseFloat(this.additionalPlans[i].weeklyIndemnity).toFixed(2);
        this.calculatePayoutAmount(i, 'weekly');
      }
    }
    if (type === 'getWellBenefit') {
      if (!this.additionalPlans[i].getWellBenefit || this.additionalPlans[i].getWellBenefit.length === 0) {
        this.additionalPlans[i].getWellBenefit = 0;
      } else {
        this.additionalPlans[i].getWellBenefit = parseFloat(this.additionalPlans[i].getWellBenefit).toFixed(2);
      }
    }
    this.calculateClaimableAmount(i);
  }
  public generatePdfAndShare() {
    this.dataService.present().then((a) => {
      a.present();
      this.fileName = 'ISP_Calculation.pdf';
      const element = document.getElementById('isp-pdf-wrap');
      const opt = {
        margin: [30, 10],
        filename: this.fileName,
        html2canvas: {
          useCORS: true,
          // backgroundColor: null,
          scale: 1,
        },
        image: { type: 'jpg', quality: 0.95 },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
        // pagebreak: { mode: ['css', 'legacy'] }
      };
      const that = this;
      html2pdf()
        .set(opt)
        .from(element)
        .toPdf()
        .get('pdf')
        .then((pdf) => {
          const totalPages = pdf.internal.getNumberOfPages();
          // tslint:disable-next-line: max-line-length
          const watermarkBase64 = 'data:image/png;base64,'
            + ''
            + ''
            + '';
          for (let i = 0; i < totalPages; i++) {
            pdf.setPage(i);
            const width = pdf.internal.pageSize.getWidth();
            const height = pdf.internal.pageSize.getHeight();
            // pdf.addImage(that.watermarkImg, 'PNG', 0, 0, width, height);
            pdf.addImage(watermarkBase64, 'PNG', 0, 0);
          }
        })
        .output('datauristring').then((resp) => {
          // 
          this.file = resp;
          this.shareSocialMedia();
        });
    });
  }
  public generatePdfAndDownload() {
    this.dataService.present().then((a) => {
      a.present();
      this.fileName = 'ISP_Calculation.pdf';
      const element = document.getElementById('isp-pdf-wrap');
      const opt = {
        margin: [30, 10],
        filename: this.fileName,
        html2canvas: {
          useCORS: true,
          // backgroundColor: null,
          scale: 1,
        },
        image: { type: 'jpg', quality: 0.95 },
        jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
        // pagebreak: { mode: ['css', 'legacy'] }
      };
      const that = this;
      html2pdf()
        .set(opt)
        .from(element)
        .toPdf()
        .get('pdf')
        .then((pdf) => {
          const totalPages = pdf.internal.getNumberOfPages();
          // tslint:disable-next-line: max-line-length
          const watermarkBase64 = 'data:image/png;base64,';
          for (let i = 0; i < totalPages; i++) {
            pdf.setPage(i);
            const width = pdf.internal.pageSize.getWidth();
            const height = pdf.internal.pageSize.getHeight();
            // pdf.addImage(that.watermarkImg, 'PNG', 0, 0, width, height);
            pdf.addImage(watermarkBase64, 'PNG', 0, 0, width, height);
          }
        }).save().then((resp) => {
          this.dataService.dismiss();
        });
    });
  }
  public shareSocialMedia() {
    if (this.file.includes('filename=generated.pdf;')) {
      this.file = this.file.replace('filename=generated.pdf;', 'filename=' + this.fileName + ';');
    }
    if (this.file.includes('filename=attachment.pdf;')) {
      this.file = this.file.replace('filename=attachment.pdf;', 'filename=' + this.fileName + ';');
    }
    if (!this.internetCheckFlag) {
      this.socialSharing.share('ISP Calculation', 'ISP Calculation', this.file, null)
        .then((resp) => {
          this.dataService.dismiss();
        })
        .catch((err) => {
          this.dataService.dismiss();
        });
    } else {
      this.dataService.dismiss();
      this.dataService.presentAlert('Please check your internet connection!');
    }
  }
  public calculateRiderBasicDeductible() {
    const total = this.basicDeductible * this.nonPayableDeductible;
    this.riderBasicDeductible = total.toFixed(2);
  }
}
